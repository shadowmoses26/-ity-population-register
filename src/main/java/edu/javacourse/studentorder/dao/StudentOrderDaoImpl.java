...
public class StudentOrderDaoImpl implements StudentOrderDao {
    ...
    private static final String INSERT_ORDER =
            "INSERT INTO public.jc_student_order(" +
                    "student_order_status, student_order_date, h_sur_name, h_given_name, h_patronymic, h_date_of_birth, h_passport_serial, h_passport_number, h_passport_date, h_passport_office_id, h_post_index, h_street_code, h_building, h_extension, h_apartment, h_university_id, h_student_number, w_sur_name, w_given_name, w_patronymic, w_date_of_birth, w_passport_serial, w_passport_number, w_passport_date, w_passport_office_id, w_post_index, w_street_code, w_building, w_extension, w_apartment, w_university_id, w_student_number, certificate_id, register_office_id, marriage_date)" +
                    "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);";
    ...
    private static void setParamsForAdult(PreparedStatement stmt, Adult adult, int start) throws SQLException {
    ...
    private static void setParamsForPerson(PreparedStatement stmt, Person person, int start)
            throws SQLException {
    ...
    private static void setParamsForAddress(PreparedStatement stmt, Person person, int start) throws SQLException {
    ...
    private void findChildren(Connection con, List<StudentOrder> result) throws SQLException {
        ...
        Map<Long, StudentOrder> maps =
                result.stream().collect(Collectors.toMap(so -> so.getStudentOrderId(), so -> so));
        ...
        try (PreparedStatement stmt = con.prepareStatement(SELECT_CHILD + cl)) {
            ...
            while (rs.next()) {
                ...
                StudentOrder so =
                        maps.get(rs.getLong("student_order_id"));
                so.addChild(ch);
            }
        }
    ...
    private Adult fillAdult(ResultSet rs, String pref) throws SQLException {
        ...
        adult.setPassportSeria(rs.getString(pref + "passport_serial"));
